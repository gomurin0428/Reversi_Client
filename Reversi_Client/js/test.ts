...
class Board{
    ...
    async drawPlayer(newPlayer:string,oldPlayer:string,oldOppoent:string){
        for(let i=0;i<6;i++){
            for(let j=0;j<6;j++){
                var index = 6*i+j;
    ...
    async drawOpponent(newOpponent:string,oldPlayer:string,oldOppoent:string){
        for(let i=0;i<6;i++){
            for(let j=0;j<6;j++){
                var index = 6*i+j;
    ...
    drawReversingAnimation(count:number,countMax:number,index:number,newColor:string,oldColor:string){
        const p = new Promise((resolve,reject)=>{
            if(count > countMax){
                resolve(1);
            }
